<x-layouts.dashboard.master>
    ...
    <div class="card">
        <div
            ...
            @can('create-role')
                <x-button class="btn-primary-600" :href="route('dashboard.roles.create')">
            ...
            <div class="card-body p-24">
                <div class="table-responsive scroll-sm">
                    <table class="table bordered-table sm-table mb-0">
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex align-items-center gap-10 justify-content-center">
                                            @can('show-role')
                                                <x-table.actions.show route="{{ route('dashboard.roles.show', $role) }}"
                                                    :model="$role" />
                                @endif

                                @can('update-role')
                                    <x-table.actions.edit route="{{ route('dashboard.roles.edit', $role) }}" :model="$role" />
                                    @endif

                                    @can('delete-role')
                                        <x-table.actions.delete route="{{ route('dashboard.roles.destroy', $role) }}"
                                            :model="$role" />
                                        @endif
                        ...
                        <x-table.pagination :data="$roles" class="mt-3" />
                    </div>
                    </div>
                </x-layouts.dashboard.master>
