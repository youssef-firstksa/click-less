<div class="row gy-3">
    @foreach (LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
        <div class="col-lg-6 mb-4">
            <x-form.label for="title">{{ __('dashboard.sections_management.form.title') }}
                ({{ $properties['native'] }})
    ...
    <div class="col-lg-6">
        <x-form.label for="bank_id">{{ __('dashboard.sections_management.form.bank') }}</x-dashboard.form.label>

            <x-form.select2 id="bank_id" name="bank_id" :options="$bankOptions"
                placeholder="{{ __('dashboard.general.select') }}" :selected="$section?->bank_id" data-cascade-element="product_id"
    ...
    <div class="col-lg-6">
        <x-form.label for="product_id">{{ __('dashboard.sections_management.form.product') }}</x-dashboard.form.label>
            ...
            <x-form.select2 id="product_id" name="product_id" :options="$productOptions"
                placeholder="{{ __('dashboard.general.select') }}" :selected="$section?->product_id" :disabled="!$section?->product_id" />

            <x-form.error :messages="$errors->get('product_id')" />
    </div>

    <div class="col-lg-6">
        <x-form.label for="status">{{ __('dashboard.sections_management.form.status') }}</x-dashboard.form.label>
