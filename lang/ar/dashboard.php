<?php

return [
    'general' => [
        'dashboard' => 'لوحة التحكم',
        'action' => 'إجراء',
        'create' => 'إنشاء',
        'edit' => 'تعديل',
        'delete' => 'حذف',
        'submit' => 'إرسال',
        'add' => 'إضافة',
        'remove' => 'إزالة',
        'update' => 'تحديث',
        'add_new' => 'إضافة جديد',
        'show' => 'عرض',
        'activated' => 'مفعل',
        'disabled' => 'معطل',
        'status' => 'الحالة',
        'table_id' => '#',
        'search' => 'بحث',
        'all' => 'الكل',
        'select' => 'اختر',
        'back' => 'الرجوع',
        'confirm' => 'تأكيد',
        'cancel' => 'الغاء',
        'see_all_message' => 'عرض جميع الرسائل',
        'notifications' => 'الإشعارات',
        'see_all_notifications' => 'عرض جميع الإشعارات',
        'messages' => 'الرسائل',
        'choose_your_language' => 'اختر لغتك',
        'english' => 'الإنجليزية',
        'arabic' => 'العربية',
        'my_profile' => 'ملفي الشخصي',
        'inbox' => 'صندوق الوارد',
        'settings' => 'الإعدادات',
        'logout' => 'تسجيل الخروج',
        'copy_right' => 'جميع الحقوق محفوظة © 2024 Upsource.',
        'success' => 'نجاح',
        'info' => 'معلومات',
        'warning' => 'تحذير',
        'error' => 'حطأ',
        'popup' => 'منبثق',
        'top_bar' => 'شريط علوي',
    ],
    'messages' => [
        'success' => [
            'title' => 'تم بنجاح!',
            'created' => 'تم إنشاء :resource بنجاح',
            'updated' => 'تم تحديث :resource بنجاح',
            'deleted' => 'تم حذف :resource بنجاح',
        ],
        'error' => [
            'title' => 'حدث خطأ!',
            'create' => 'فشل في إنشاء :resource',
            'update' => 'فشل في تحديث :resource',
            'delete' => 'فشل في حذف :resource',
        ],
        'confirm' => [
            'delete' => [
                'title' => 'تأكيد عملية الحذف',
                'description' => 'هل أنت متأكد أنك تريد حذف هذا العنصر؟',
            ],
        ],
    ],
    'sidebar' => [
        'dashboard' => 'لوحة التحكم',
        'permissions_management' => 'إدارة الصلاحيات',
        'content_management' => 'إدارة المحتوى',
        'bank_management' => 'إدارة بنك المعرفة',
        'notifications_management' => 'إدارة الإشعارات',
    ],
    'permissions_management' => [
        'index_title' => 'قائمة الصلاحيات',
        'create_title' => 'إضافة صلاحية جديدة',
        'edit_title' => 'تعديل الصلاحية',
        'delete_title' => 'حذف الصلاحية',
        'show_title' => 'عرض الصلاحية',

        'form' => [
            'title' => 'العنوان',
            'description' => 'الوصف',
            'status' => 'الحالة',
        ],
    ],
    'users_management' => [
        'index_title' => 'قائمة المستخدمين',
        'create_title' => 'إضافة مستخدم جديد',
        'edit_title' => 'تعديل بيانات المستخدم',
        'delete_title' => 'حذف المستخدم',
        'show_title' => 'عرض المستخدم',

        'form' => [
            'name' => 'الاسم',
            'email' => 'البريد الإلكتروني',
            'password' => 'كلمة المرور',
            'password_confirmation' => 'تأكيد كلمة المرور',
            'role' => 'الدور',
            'status' => 'الحالة',
        ],
    ],
    'bank_management' => [
        'index_title' => 'قائمة البنوك',
        'create_title' => 'إنشاء بنك جديد',
        'edit_title' => 'تعديل بنك',
        'delete_title' => 'حذف البنك',
        'show_title' => 'عرض بنك',
        'form' => [

            'title' => 'العنوان',
            'description' => 'وصف البنك',
            'font_color' => 'لون الخط',
            'background_color' => 'لون الخلفية',
            'logo' => 'الشعار',
            'image' => 'الصورة',
            'ai_key' => 'مفتاح الذكاء الاصطناعي',
            'status' => 'الحالة',
        ],
    ],
    'products_management' => [
        'index_title' => 'قائمة المنتجات',
        'create_title' => 'إنشاء منتج جديد',
        'edit_title' => 'تعديل المنتج',
        'delete_title' => 'حذف المنتج',
        'show_title' => 'عرض المنتج',

        'form' => [
            'title' => 'العنوان',
            'status' => 'الحالة',
            'bank' => 'البنك',
        ],
    ],
    'sections_management' => [
        'index_title' => 'قائمة أقسام المنتجات',
        'create_title' => 'إنشاء قسم جديد',
        'edit_title' => 'تعديل القسم',
        'delete_title' => 'حذف القسم',
        'show_title' => 'عرض القسم',

        'form' => [
            'title' => 'العنوان',
            'status' => 'الحالة',
            'bank' => 'البنك',
            'product' => 'المنتج',
        ],
    ],
    'articles_management' => [
        'index_title' => 'قائمة المقالات',
        'create_title' => 'إنشاء مقال جديد',
        'edit_title' => 'تعديل المقال',
        'delete_title' => 'حذف المقال',
        'show_title' => 'عرض المقال',

        'form' => [
            'title' => 'العنوان',
            'content' => 'المحتوى',
            'status' => 'الحالة',
            'bank' => 'البنك',
            'product' => 'المنتج',
            'section' => 'القسم',
            'published_at' => 'تاريخ النشر',
            'author' => 'الناشر',
        ],
    ],
    'notifications_management' => [
        'index_title' => 'قائمة الإشعارات',
        'create_title' => 'إنشاء إشعار جديد',
        'edit_title' => 'تعديل الإشعار',
        'delete_title' => 'حذف الإشعار',
        'show_title' => 'عرض الإشعار',

        'form' => [
            'title' => 'العنوان',
            'description' => 'التفاصيل',
            'status' => 'الحالة',
            'bank' => 'البنك',
            'notifications_duration' => 'تحديد جدول الإشعار',
            'notifications_duration_text' => 'بعد إنتهاء جدول الإشعار سيتم تغيير حالة الإشعار إلى منتهي',
            'publish_start_at' => 'تاريخ البدء',
            'publish_end_at' => 'تاريخ الانتهاء',
            'notification_type' => 'نوع الإشعار',
            'notification_view' => 'طريقة العرض',
            'type_success' => 'إشعار نجاح',
            'type_error' => 'إشعار مشكلة',
            'type_warning' => 'إشعار تنبيه',
            'type_info' => 'إشعار معلومة',
            'prop_details' => 'مزيد من التفاصيل',
            'prop_confirm' => 'تأكيد الإطلاع',
            'prop_popup' => 'Pop up',
            'prop_quick' => 'اختيار سريع',
        ],
    ],
];
